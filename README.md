...
## コマンド：「GTool:半角英数字を全角に」
 * guns-tool.halfwidthToFullwidth

### 機能：

 * 選択範囲がある場合はその範囲内、選択範囲がない場合はドキュメント全体の半角英数字・記号・スペース（ASCII 0x20-0x7E）を全角に変換します。
...
<a id="guns-tool.fullwidthToHalfwidth"></a>

## コマンド：「GTool:選択範囲内の全角英数字を半角に」
 * guns-tool.fullwidthToHalfwidth

### 機能：

 * ドキュメント内で選択したテキスト内の全角英数字・記号・スペース（0xFF00-0xFF5E）を半角に変換します。

#### 動作例

選択： `Ｈｅｌｌｏ１２３` を選択した状態でコマンドを実行すると：

出力: `Hello123`

選択： `ＡＢＣ－ｘｙｚ－７８９` を選択した状態でコマンドを実行すると：

出力: `ABC-xyz-789`（ハイフンも変換されます）

選択： `ｔｅｓｔ　２０２４` を選択した状態でコマンドを実行すると：

出力: `test 2024`（全角スペースも半角スペースに変換されます）

選択： `Ａ　＋　Ｂ　＝　Ｃ` を選択した状態でコマンドを実行すると：

出力: `A + B = C`（記号とスペースも半角に変換されます）

複数選択に対しては各選択範囲それぞれが半角化されます。
...
- 「なろう形式傍点挿入」コマンドを追加
### 以降、CHANGELOG形式
- v0.1.92 「半角数字を全角数字に」コマンドを追加
- v0.1.93 「漢数字を全角数字に」コマンドを追加
- v0.1.94  「選択範囲内の全角英数字を半角に」コマンドを追加
- v0.1.95  「選択範囲内の全角英数字を半角に」コマンドから「選択範囲内」限定を解除
- v0.1.96  「文書ステイタス表示」コマンドに選択範囲内文字数を追加
